...
PlotAllWindowSizes("COVARIATE", 1, "../../../../../data_out/airlines/moving_chunk/", c())
PlotAllWindowSizes("LIKELIHOOD", 1, "../../../../../data_out/airlines/moving_chunk/", c())
PlotAllWindowSizes("POSTERIOR", 1, "../../../../../data_out/airlines/moving_chunk/", c())
PlotAllWindowSizes("JOINT", 1, "../../../../../data_out/airlines/moving_chunk/", c())
```
...
```{r}
data.all <- read.arff("../../../../../datasets/airlines.arff")
Histogram(data.all[seq(0,240796),], data.all[seq(240797,481594),])
```

#### Between first 2 windows with no drift
```{r}
Histogram(data.all[seq(0,17612),], data.all[seq(17613,35659),])
```

#### The first drift peak in the timeline
```{r}
Histogram(data.all[seq(35660,53799),], data.all[seq(53800,68078),])
Histogram(data.all[seq(53800,68078),], data.all[seq(68079,85189),])
```

We can observe that there is a big difference between Friday and Saturday and between Saturday and Sunday.

##### The third drift peak in the timeline
```{r}
Histogram(data.all[seq(276643,295227),], data.all[seq(295228,311213),])
Histogram(data.all[seq(295228,311213),], data.all[seq(311214,328765),])
```